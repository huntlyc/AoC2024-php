...
EOT;
    ...
    function minA(int $initAvalue, int $numOuputsSoFar, array $currentProg): int{
        if ($numOuputsSoFar > count($currentProg)) {
            return $initAvalue;
        }

        // someone smarter than me on reddit figured this out out. I gave in tonight...
        for ($i = 0; $i < 8; $i++) {


            $nextA = $initAvalue << 3 | $i;

            $comp = new Computer($nextA, 0, 0, $currentProg);
            $out = array_map('intval', explode(',', $comp->runProgram()));

            if ($out === array_slice($currentProg, -$numOuputsSoFar)) {
                $result = minA($nextA, $numOuputsSoFar+1, $currentProg);
                if ($result !== -1) {
                    return $result;
                }
            }
    ...
    function main():void{
        ...
        //$input = file_get_contents(__DIR__ . '/part2-test.txt');
