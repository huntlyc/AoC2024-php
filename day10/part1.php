...
class PathFinder
{
    ...
    public function solve(): int
    {
        ...
        foreach ($this->startingPositions as $startingPosition) {
            ...
            $this->findPathFromPoint($startingPosition, $startingPosition);
            if (!empty($this->endingPositions)) {
                var_dump($this->endingPositions);
                foreach (
                    $this->endingPositions
                    as $origin => $endingPositions
                ) {
                    $ans += count($endingPositions);
                }
            }
        }
        return $ans;
     ...
     * @return bool
     */
    private function findPathFromPoint(
        Point $origin,
        ...
        if (
            $this->grid->getGrid()[$startingPosition->y][
                $startingPosition->x
            ] === "9"
        ) {
            echo ">>>{$origin} - found end at {$startingPosition}" . PHP_EOL;
            $currentEndPointsFromOrigin = null;

            if (isset($this->endingPositions["$origin"])) {
                $currentEndPointsFromOrigin = $this->endingPositions["$origin"];
            }

            if (is_null($currentEndPointsFromOrigin)) {
                $this->endingPositions["$origin"][] = $startingPosition;
                return true;
            }

            if (!in_array($startingPosition, $currentEndPointsFromOrigin)) {
                $this->endingPositions["$origin"][] = $startingPosition;
            }
        ...
        if ($this->isPointOutOfBounds($startingPosition, $this->grid)) {
            //echo "OOB at {$this->grid->getGrid()[$startingPosition->y][$startingPosition->x]}" . PHP_EOL;
            return false;
        }
        ...
        foreach ($validMoves as $move) {
            $this->findPathFromPoint($origin, $move, $ans);
        }
    ...
    private function isPointOutOfBounds(Point $point, Grid $grid): bool
    {
        ...
        $y = $point->y;
        return $x < 0 ||
            $x >= count($grid->getGrid()[0]) ||
            $y < 0 ||
            $y >= count($grid->getGrid());
    }
}
...
class Part1
{
    static function run(): void
    {
        ...
        if ($input === false) {
            exit("Input file not found" . PHP_EOL);
        }
