<?php
    ...
    class Grid
    {
        ...
        public function positionOutOfBound(Position $pos){
            return(($pos->y < 0 || $pos->y >= count($this->grid)) || ($pos->x < 0 || $pos->x >= count($this->grid[0])));
        }
        ...
        /**
         * Create an empty grid of a size
         * @param string $input
         * @return Grid
         */
        static function emptyGrid(int $x, int $y): Grid
        {
            $grid = [];
            for($r = 0; $r <= $y; $r++){
                $newRow = [];
        ...
        public function getShortestPathBetween(Position $start, Position $end): int{
            ...
            foreach($this->queue as ["data" => $pathPoint, "priority" => $priority]){
                ...
                if($pathPoint->position == $end){
                    var_dump($this->queue);
                    echo count($this->visitedPositions);
                    return (int) $cost;
                }
                ...
                foreach($directions as $dir){
                    ...
                    if($this->grid->positionOutOfBound($dest)){
                        echo $dest . " is oob " .PHP_EOL;
                        continue;
                    }
    ...
    function main():void{
        ...
        $size = 69;
        $grid = Grid::emptyGrid($size,$size);
        $start = new Position(0,0);
        $end = new Position($size,$size);


        $corrupted = [];
        foreach($lines as $line){
            $p = explode(',', $line);
            $corrupted[] = new Position($p[0],$p[1]);
        }
